...
import { conversations, knowledgeDocument } from "./schema";
...
export async function createArtifact(userId:string,input:{conversationId?:string|null;sourceDocumentId?:string|null;title:string;content:string}){
  if (input.sourceDocumentId) {
    const [source] = await db.select({ id: knowledgeDocument.id }).from(knowledgeDocument).where(and(eq(knowledgeDocument.id,input.sourceDocumentId),eq(knowledgeDocument.userId,userId)));
    if (!source) return null;
  }
  const [row]=await db.insert(artifacts).values({userId,conversationId:input.conversationId??null,sourceDocumentId:input.sourceDocumentId??null,title:input.title,content:input.content}).returning();
  await db.insert(artifactVersions).values({artifactId:row.id,userId,title:row.title,content:row.content});
  return row;
}
