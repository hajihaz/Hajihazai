...
export async function listAutomations(userId: string, projectId?: string) {
  return db
    ...
    .where(
      projectId
        ? and(eq(automations.userId, userId), eq(automations.projectId, projectId))
...
export async function updateAutomation(userId: string, id: string, patch: {
  ...
  if (patch.projectId) {
    const [project] = await db
      .select({ id: projects.id })
      .from(projects)
      .where(and(eq(projects.id, patch.projectId), eq(projects.userId, userId)))
      .limit(1);
    if (!project) return null;
  }
  const [row] = await db
